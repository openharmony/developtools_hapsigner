...
std::unique_ptr<SegmentHeader> SegmentHeader::FromByteArray(std::vector<int8_t> bytes)
{
    if (bytes.size() != SEGMENT_HEADER_LENGTH) {
        PrintErrorNumberMsg("SIGN_ERROR", SIGN_ERROR, "The signed package has the wrong flag in the SegmentHeader");
    ...
    if ((inType != CSB_FSVERITY_INFO_SEG) && (inType != CSB_HAP_META_SEG)
        ...
        PrintErrorNumberMsg("SIGN_ERROR", SIGN_ERROR, "The signed package has the wrong type in the SegmentHeader");
        return std::unique_ptr<SegmentHeader>();
    }
    ...
    if (inSegmentSize < 0) {
        PrintErrorNumberMsg("SIGN_ERROR", SIGN_ERROR, "The signed package has the wrong segmentsize in the SegmentHeader");
        return std::unique_ptr<SegmentHeader>();
    }
