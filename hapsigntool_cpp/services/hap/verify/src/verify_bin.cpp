...
bool VerifyBin::Verify(Options* options)
{
    ...
    if (!checkSignFileFlag) {
        SIGNATURE_TOOLS_LOGE("check input bin file %{public}s failed!", filePath.c_str());
        return false;
    }
    // verify bin
    ...
    if (!verifyBinFileFLag) {
        SIGNATURE_TOOLS_LOGE("verify bin file %{public}s failed!", filePath.c_str());
        return false;
    }
                              ...
                              Options* options, Pkcs7Context& pkcs7Context)
{
    ...
    if (!getSignBlockInfoFlag) {
        SIGNATURE_TOOLS_LOGE("get signBlockInfo failed on verify bin %{public}s", binFile.c_str());
        return false;
    }
    // verify profile
    ...
    if (!verifyP7bFlag) {
        SIGNATURE_TOOLS_LOGE("verify profile failed on verify bin %{public}s", binFile.c_str());
        return false;
    }
    // verify signed data
    bool verifyBinDigestFlag = VerifyBinDigest(signBlockInfo);
    if (!verifyBinDigestFlag) {
        SIGNATURE_TOOLS_LOGE("verify digest failed on verify bin %{public}s", binFile.c_str());
        return false;
    }
    return true;
}
