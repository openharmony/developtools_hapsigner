...
{
    SIGNATURE_TOOLS_LOGE("%{public}s", std::string("(Error Code: " + std::to_string(errorCode) + ")" + log).c_str());
    if (path != "") {
        remove(path.c_str());
    }
    return false;
...
bool SignProvider::CheckSignatureAlg()
{
    ...
    PrintErrorNumberMsg("COMMAND_PARAM_ERROR", COMMAND_PARAM_ERROR,
                        "signAlg Parameter is not support");
    return false;
}
...
bool SignProvider::CheckCompatibleVersion()
{
    ...
    if (!CheckStringToint(compatibleApiVersionVal, compatibleApiVersion)) {
        PrintErrorNumberMsg("COMMAND_PARAM_ERROR", COMMAND_PARAM_ERROR,
