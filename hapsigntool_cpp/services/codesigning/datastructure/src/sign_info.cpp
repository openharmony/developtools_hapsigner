...
SignInfo SignInfo::FromByteArray(std::vector<int8_t> bytes)
{
    ...
    if (inSaltSize < 0 || inSigSize < 0) {
        SIGNATURE_TOOLS_LOGE("Invalid saltSize or sigSize of SignInfo, saltSize: %{public}d, sigSize: %{public}d",
    ...
    if (inFlags != 0 && inFlags != SignInfo::FLAG_MERKLE_TREE_INCLUDED) {
        SIGNATURE_TOOLS_LOGE("Invalid flags of SignInfo: %{public}d", inFlags);
        return SignInfo();
    }
    ...
    if (inExtensionNum < 0 || inExtensionNum > SignInfo::MAX_EXTENSION_NUM) {
        SIGNATURE_TOOLS_LOGE("Invalid extensionNum of SignInfo: %{public}d", inExtensionNum);
        return SignInfo();
    }
    ...
    if (inExtensionOffset < 0 || inExtensionOffset % SignInfo::SIGNATURE_ALIGNMENT != 0) {
        SIGNATURE_TOOLS_LOGE("Invalid extensionOffset of SignInfo: %{public}d", inExtensionOffset);
        return SignInfo();
    }
