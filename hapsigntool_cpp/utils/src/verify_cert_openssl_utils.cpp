...
X509* VerifyCertOpensslUtils::GetX509CertFromBase64String(const std::string& base64String)
{
    ...
    if (len <= 0) {
        VerifyHapOpensslUtils::GetOpensslErrorMessage();
        SIGNATURE_TOOLS_LOGE("base64Decode failed, len: %d", len);
        return nullptr;
    }
                                                             ...
                                                             std::string& fingerprint)
{
    ...
    X509_free(cert);
    SIGNATURE_TOOLS_LOGD("GetFingerprintBase64FromPemCert end %s", fingerprint.c_str());
    return true;
}
...
bool VerifyCertOpensslUtils::GetPublickeyBase64(const X509* cert, std::string& publicKey)
{
    ...
    if (keyLen <= 0) {
        SIGNATURE_TOOLS_LOGE("keyLen %d, i2d_PublicKey failed", keyLen);
        VerifyHapOpensslUtils::GetOpensslErrorMessage();
        return false;
    }
                                                          ...
                                                          int32_t offset, int32_t len)
{
    ...
    if ((len <= 0) || (offset < 0) || (crlBuffer.GetCapacity() - len < offset)) {
        SIGNATURE_TOOLS_LOGE("invalid input, offset: %d, len: %d", offset, len);
        return nullptr;
    }
...
bool VerifyCertOpensslUtils::GetSerialNumberFromX509(const X509* cert, int64_t& certNumber)
{
    ...
    SIGNATURE_TOOLS_LOGD("cert number = %" PRId64, certNumber);
    return true;
}
