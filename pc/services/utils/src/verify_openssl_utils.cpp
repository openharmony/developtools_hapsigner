     ...
     */
        ...
        bool HapVerifyOpensslUtils::GetCertChains(PKCS7* p7, Pkcs7Context& pkcs7Context)
        {
            ...
            for (int32_t i = 0; i < signCount; i++) {
                ...
                if (signInfo == nullptr) {
                    SIGNATURE_TOOLS_LOGE("signInfo %{public}dst is nullptr", i);
                    return false;
                }
                /* GET X509 certificate */
                X509* cert = PKCS7_cert_from_signer_info(p7, signInfo);
                if (cert == nullptr) {
                    SIGNATURE_TOOLS_LOGE("get cert for %{public}dst signInfo failed", i);
                    return false;
                }
                CertChain certChain;
                ...
                if (!VerifyCertChain(pkcs7Context.certChains[i], p7, signInfo, pkcs7Context, certVisitSign)) {
                    SIGNATURE_TOOLS_LOGE("verify %{public}dst certchain failed", i);
                    return false;
                }
            }
            return true;
        ...
        bool HapVerifyOpensslUtils::VerifyPkcs7SignedData(Pkcs7Context& pkcs7Context)
        {
            ...
            for (int32_t i = 0; i < signCount; i++) {
                if (!VerifySignInfo(signerInfoStack, p7Bio, i, pkcs7Context)) {
                    SIGNATURE_TOOLS_LOGE("Verify %{public}dst signInfo failed", i);
                    BIO_free_all(p7Bio);
                    return false;
                }
            }
                                                   ...
                                                   BIO* p7Bio, int32_t signInfoNum, Pkcs7Context& pkcs7Context)
        {
            ...
            if (signInfo == nullptr) {
                SIGNATURE_TOOLS_LOGE("signInfo %{public}dst is nullptr", signInfoNum);
                return false;
            }
            /* GET X509 certificate */
            ...
            if (PKCS7_signatureVerify(p7Bio, pkcs7Context.p7, signInfo, cert) <= 0) {
                SIGNATURE_TOOLS_LOGE("PKCS7_signatureVerify %{public}dst signInfo failed", signInfoNum);
                GetOpensslErrorMessage();
                return false;
            }
        ...
        const EVP_MD_CTX* HapVerifyOpensslUtils::FindMdCtxInBio(BIO* p7Bio, int32_t mdType)
        {
            ...
            while (p7Bio) {
                ...
                if (mdCtx == nullptr) {
                    SIGNATURE_TOOLS_LOGE("Get null from bio");
                    return nullptr;
                }
        ...
        bool HapVerifyOpensslUtils::VerifyPkcs7AuthAttributes(const PKCS7_SIGNER_INFO* signInfo,
                                                              EVP_MD_CTX* mdCtx, int32_t mdType)
        {
            ...
            if ((authAttributes != nullptr) && (sk_X509_ATTRIBUTE_num(authAttributes) != 0)) {
                ...
                if (EVP_DigestFinal_ex(mdCtx, digest, &digestLen) <= 0) {
                    SIGNATURE_TOOLS_LOGE("Digest content failed");
                    GetOpensslErrorMessage();
                    return false;
                }
                ...
                if (!AsnStringCmp(digestInAttribute, digest, static_cast<int>(digestLen))) {
                    SIGNATURE_TOOLS_LOGE("AsnStringCmp failed");
                    return false;
                }
                if (!EVP_VerifyInit_ex(mdCtx, EVP_get_digestbynid(mdType), nullptr)) {
                    SIGNATURE_TOOLS_LOGE("EVP_VerifyInit_ex failed");
                    GetOpensslErrorMessage();
                    return false;
                }
                ...
                if (attributesLen <= 0 || attributesData == nullptr) {
                    SIGNATURE_TOOLS_LOGE("ASN1_item_i2d failed");
                    GetOpensslErrorMessage();
                    return false;
                }
                if (!EVP_VerifyUpdate(mdCtx, attributesData, attributesLen)) {
                    SIGNATURE_TOOLS_LOGE("EVP_VerifyUpdate failed");
                    GetOpensslErrorMessage();
        ...
        bool HapVerifyOpensslUtils::AsnStringCmp(const ASN1_OCTET_STRING* asnStr,
                                                 const unsigned char data[], int32_t len)
        {
            if (asnStr == nullptr) {
                SIGNATURE_TOOLS_LOGE("asnStr is nullptr");
                return false;
            }
            if (asnStr->data == nullptr) {
                SIGNATURE_TOOLS_LOGE("asnStr->data is nullptr");
                return false;
            }
            if (data == nullptr) {
                SIGNATURE_TOOLS_LOGE("data is nullptr");
                return false;
            }
            if (asnStr->length != len) {
                SIGNATURE_TOOLS_LOGE("asnStr->length: %{public}d is not equal to len: %{public}d", asnStr->length, len);
                return false;
            }
            for (int32_t i = 0; i < len; i++) {
                if (asnStr->data[i] != data[i]) {
                    SIGNATURE_TOOLS_LOGE("%{public}dst data is not equal", i);
                    return false;
                }
            }
            return true;
                                                  ...
                                                  std::vector<std::string>& SignatureVec)
        {
            for (uint32_t i = 0; i < signCertChain.size(); i++) {
                if (!GetPublickeyFromCertificate(signCertChain[i], SignatureVec)) {
                    SIGNATURE_TOOLS_LOGE("%{public}ust Get Publickey failed", i);
                    return false;
                }
            }
                                                  ...
                                                  std::vector<std::string>& SignatureVec)
        {
            for (uint32_t i = 0; i < signCertChain.size(); i++) {
                if (!GetDerCert(signCertChain[i], SignatureVec)) {
                    SIGNATURE_TOOLS_LOGE("%{public}ust GetDerCert failed", i);
                    return false;
                }
            }
            return !SignatureVec.empty();
        ...
        bool HapVerifyOpensslUtils::GetPublickeyFromCertificate(const X509* ptrX509,
        ...
        bool HapVerifyOpensslUtils::GetContentInfo(const PKCS7* p7ContentInfo, ByteBuffer& content)
        {
            ...
            SIGNATURE_TOOLS_LOGD("strContentInfoLen: %{public}d", strContentInfoLen);
            return true;
        }
        ...
        int32_t HapVerifyOpensslUtils::GetDigest(const DigestParameter& digestParameter,
        ...
        int32_t HapVerifyOpensslUtils::GetDigest(const ByteBuffer& chunk,
                                                 ...
                                                 unsigned char(&out)[EVP_MAX_MD_SIZE])
        {
            ...
            for (int32_t i = 0; i < static_cast<int>(optionalBlocks.size()); i++) {
                ...
                if (EVP_DigestUpdate(digestParameter.ptrCtx, optionalBlocks[i].optionalBlockValue.GetBufferPtr(),
                    chunkLen) <= 0) {
                    GetOpensslErrorMessage();
                    SIGNATURE_TOOLS_LOGE("EVP_DigestUpdate %{public}dst optional block failed", i);
                    return outLen;
                }
            }
            if (EVP_DigestFinal(digestParameter.ptrCtx, out, &outLen) <= 0) {
        ...
        void HapVerifyOpensslUtils::GetOpensslErrorMessage()
        {
            ...
            while ((retOpenssl = ERR_get_error()) != 0) {
                ...
                SIGNATURE_TOOLS_LOGE("openssl err: %{public}lu, message: %{public}s", retOpenssl, errOpenssl);
            }
        }
        ...
        int32_t HapVerifyOpensslUtils::GetDigestAlgorithmId(int32_t signAlgorithm)
        {
            switch (signAlgorithm) {
                ...
                case ALGORITHM_SHA512_WITH_DSA:
                    return NID_sha512;
                default:
                    SIGNATURE_TOOLS_LOGE("signAlgorithm: %{public}d error", signAlgorithm);
                    return NID_undef;
            }
        }
    } // namespace SignatureTools
