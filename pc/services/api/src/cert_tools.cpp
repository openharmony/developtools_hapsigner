                                     ...
                                     EVP_PKEY* keyPair, Options* options)
{
    ...
    int validity = 0;
    X509* cert = X509_new();
    if (!SerialNumberBuilder(&serialNumber) ||
...
bool CertTools::SetExpandedInfExtThree(X509* cert, Options* options,
                                       std::string critical, X509_EXTENSION* ext)
{
    ...
    if (basicConstraints) {
        if (basicConstraintsCritical) {
            ext = basicConstraintsCa ? X509V3_EXT_conf(NULL, NULL, "basicConstraints", "critical,CA:TRUE") :
                X509V3_EXT_conf(NULL, NULL, "basicConstraints", "critical,CA:FALSE");
        } else {
            ext = basicConstraintsCa ? X509V3_EXT_conf(NULL, NULL, "basicConstraints", "CA:TRUE") :
                X509V3_EXT_conf(NULL, NULL, "basicConstraints", "CA:FALSE");
        }
    } else {
        if (basicConstraintsCritical) {
            ext = basicConstraintsCa ? X509V3_EXT_conf(NULL, NULL, "", "critical,CA:TRUE") :
                X509V3_EXT_conf(NULL, NULL, "", "critical,CA:FALSE");
        } else {
            ext = basicConstraintsCa ? X509V3_EXT_conf(NULL, NULL, "", "CA:TRUE") :
                X509V3_EXT_conf(NULL, NULL, "", "CA:FALSE");
        }
    }
    X509_add_ext(cert, ext, -1);
    X509_EXTENSION_free(ext);
...
bool CertTools::SetExpandedInformation(X509* cert, Options* options)
{
    ...
    if (!SetExpandedInfExtOne(cert, options, critical, ext1) ||
...
{
    if (cert == nullptr || issuercsr == nullptr || certReq == nullptr || keyPair == nullptr) {
        SIGNATURE_TOOLS_LOGE("general cert sign failed \n");
        goto err;
    }
...
X509* CertTools::GenerateCert(EVP_PKEY* keyPair, X509_REQ* certReq, Options* options)
{
    long serialNumber = 0;
    int validity = 0;
    if (keyPair == nullptr || certReq == nullptr) {
        SIGNATURE_TOOLS_LOGE("failed to generate the generalcert because of keyPair or certReq is nullptr\n");
        return nullptr;
    }
                                 ...
                                 const char signCapacity[], int capacityLen)
{
    ...
    do {
        ...
        if (!SetCertVersion(cert, DEFAULT_CERT_VERSION) || !SetCertSerialNum(cert, serialNumber)) {
            break;
        }
