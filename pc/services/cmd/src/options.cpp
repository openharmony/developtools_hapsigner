...
char* Options::GetChars(const std::string key)
{
    ...
    if (charsPtr == nullptr) {
        SIGNATURE_TOOLS_LOGI("GetChars key = %{public}s  value = nullptr !", key.c_str());
        return NULL;
    }
    return *charsPtr;
}
...
std::string Options::GetString(const std::string key, std::string checkStr)
{
    ...
    if (stringPtr == nullptr || *stringPtr == "") {
        SIGNATURE_TOOLS_LOGI("GetString  key = %{public}s value = %{public}s ", key.c_str(), checkStr.c_str());
        return checkStr;
    }
    return *stringPtr;
}
...
int Options::GetInt(const std::string key)
{
    ...
    if (stringPtr == nullptr) {
        SIGNATURE_TOOLS_LOGI("GetInt  key = %{public}s value = 0 ", key.c_str());
        return 0;
    }
    return *stringPtr;
}
