...
#define MIN_CERTS_NUM 2
namespace OHOS::SignatureTools {

    const std::string PKCS7Data::SIGN_ALG_256 = "SHA256withECDSA";
    const std::string PKCS7Data::SIGN_ALG_384 = "SHA384withECDSA";
    ...
    static int ec_pkey_ctrl2(int op, long arg1, void* arg2)
    {
    ...
    int PKCS7Data::Sign(const std::string& content, std::shared_ptr<ISigner> signer,
    ...
    int PKCS7Data::PKCS7_SIGNER_INFO_sign2(PKCS7_SIGNER_INFO* si)
    {
    ...
    int PKCS7Data::do_pkcs7_signed_attrib2(PKCS7_SIGNER_INFO* si, EVP_MD_CTX* mctx)
    {
    ...
    int PKCS7Data::PKCS7_dataFinal2_sign_attr(STACK_OF(PKCS7_SIGNER_INFO)* si_sk, BIO* bio)
    {
        EVP_MD_CTX* mdc = NULL;
    ...
    int PKCS7Data::PKCS7_dataFinal2(PKCS7* p7, BIO* bio)
    {
        int ret = 0;
        ...
        if (!PKCS7_dataFinal2_sign_attr(si_sk, bio))
            goto err;
        if (!PKCS7_dataFinal2_set_content(p7, os, bio))
            goto err;
        ret = 1;
    err:
        return ret;
    ...
    int PKCS7Data::PKCS7_final2(PKCS7* p7, const std::string& content, int flags)
    {
        BIO* p7bio;
        int ret = 0;
    ...
    static int PKCS7_SIGNER_INFO_set2(PKCS7_SIGNER_INFO* p7i, X509* x509, const EVP_MD* dgst)
    {
        ...
        */
        ...
        if (!ec_pkey_ctrl2(ASN1_PKEY_CTRL_PKCS7_SIGN, 0, p7i))
            goto err;
        ret = 1;
    err:
        return ret;
    ...
    static PKCS7_SIGNER_INFO* PKCS7_add_signature2(PKCS7* p7, X509* x509, const EVP_MD* dgst)
    {
        PKCS7_SIGNER_INFO* si = NULL;

        if ((si = PKCS7_SIGNER_INFO_new()) == NULL)
            goto err;
        if (!PKCS7_SIGNER_INFO_set2(si, x509, dgst))
            goto err;
    ...
    static PKCS7_SIGNER_INFO* PKCS7_sign_add_signer2(PKCS7* p7, X509* signcert, const EVP_MD* md, int flags)
    {
        ...
        if ((si = PKCS7_add_signature2(p7, signcert, md)) == NULL) {
    ...
    PKCS7* PKCS7Data::PKCS7_sign2(X509* signcert, STACK_OF(X509)* certs, const EVP_MD* md,
                                  const std::string& content, int flags, const std::vector<PKCS7Attr>& attrs)
    {
        ...
        if (!PKCS7_sign_add_signer2(p7, signcert, md, flags)) {
        ...
        //添加额外属性信息 如owerID
        if (PKCS7AddAttribute(p7, attrs) < 0) {
            goto err;
        }

        if (PKCS7_final2(p7, content, flags))
            return p7;
