    ...
    if ((num) <= 0) {                             \
        ...
        AppProvisionVerifyResult ParseProvision(const string& appProvision, ProvisionInfo& info)
        {
            ...
            RETURN_IF_INT_IS_NON_POSITIVE(info.versionCode, "Tag version code is empty.")
                ...
                if (info.type == ProvisionType::DEBUG) {
                    RETURN_IF_STRING_IS_EMPTY(info.bundleInfo.developmentCertificate,
                    "Tag development-certificate is empty.")
                } else if (info.type == ProvisionType::RELEASE) {
                        RETURN_IF_INT_IS_NON_POSITIVE(info.distributionType,
                        "Tag app-distribution-type is empty.")
                        RETURN_IF_STRING_IS_EMPTY(info.bundleInfo.distributionCertificate,
                        "Tag distribution-certificate is empty.")
                    }
        ...
        AppProvisionVerifyResult CheckDeviceID(ProvisionInfo& info)
        {
            ...
            if (info.debugInfo.deviceIds.empty()) {
                SIGNATURE_TOOLS_LOGE("device-id list is empty.");
                return PROVISION_DEVICE_UNAUTHORIZED;
            }
            SIGNATURE_TOOLS_LOGD("number of device ids in list: %{public}u",
                static_cast<uint32_t>(info.debugInfo.deviceIds.size()));
            if (info.debugInfo.deviceIdType != VALUE_DEVICE_ID_TYPE_UDID) {
                SIGNATURE_TOOLS_LOGE("type of device ID is not supported.");
            ...
            if (ret != EC_SUCCESS) {
                SIGNATURE_TOOLS_LOGE("obtaining current device id failed (%{public}d).", static_cast<int>(ret));
                return PROVISION_DEVICE_UNAUTHORIZED;
            }
            deviceId = std::string(udid, sizeof(udid) - 1);
#endif // STANDARD_SYSTEM
            if (deviceId.empty()) {
                SIGNATURE_TOOLS_LOGE("device-id of current device is empty.");
                return PROVISION_DEVICE_UNAUTHORIZED;
            }
            return PROVISION_OK;
        }
