...
namespace OHOS {
    namespace SignatureTools {
        ...
        const std::string HAP_CRL_TEST_BACKUP_FILE_PATH = "/data/test/hapVerify/hap_crl_backup_test";
...
namespace {
    ...
    HWTEST_F(HapCrlManagerTest, WriteAndReadCrlsFileTest001, TestSize.Level1)
    {
         ...
         */
        CopyFileState ret = CopyFile(CrlManager::CRL_FILE_PATH, HAP_CRL_TEST_BACKUP_FILE_PATH);
        ...
        CrlManager& hapCrlManager = CrlManager::GetInstance();
        ...
        if (ret == NO_SOURCE_FILE) {
            ASSERT_TRUE(ClearFile(CrlManager::CRL_FILE_PATH));
        } else {
            ASSERT_TRUE(CopyFile(HAP_CRL_TEST_BACKUP_FILE_PATH, CrlManager::CRL_FILE_PATH) == COPY_SUC);
        }
    }
}
