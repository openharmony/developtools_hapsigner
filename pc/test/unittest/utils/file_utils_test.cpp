...
HWTEST_F(FileUtilsTest, AppendWriteFileByOffsetToFile001, testing::ext::TestSize.Level1)
{
    ...
    bool result = FileUtils::AppendWriteFileByOffsetToFile(inputFile, output, 0, 32);
...
HWTEST_F(FileUtilsTest, AppendWriteFileByOffsetToFile003, testing::ext::TestSize.Level1)
{
    ...
    std::string inputFile = "./utils/unsigned-linux.out";
    std::ofstream output("./utils/signed-linux.out", std::ios::binary);
    bool result = FileUtils::AppendWriteFileByOffsetToFile(inputFile, output, -1, 32);
    EXPECT_EQ(result, false);
}

/**
 * @tc.name: AppendWriteFileByOffsetToFile004
 * @tc.desc: Test function interface for SUCCESS.
 * @tc.size: MEDIUM
 * @tc.type: FUNC
 * @tc.level Level 1
 * @tc.require: SR000H63TL
 */
HWTEST_F(FileUtilsTest, AppendWriteFileByOffsetToFile004, testing::ext::TestSize.Level1)
{
    // go to all branch
    std::string inputFile = "./utils/unsigned-linux.out";
    std::ofstream output("./utils/signed-linux.out", std::ios::binary);
