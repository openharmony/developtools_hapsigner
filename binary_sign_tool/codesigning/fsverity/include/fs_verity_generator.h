...
public:
    ...
    bool GenerateFsVerityDigest(std::istream& inputStream, long size, int flags);
    std::vector<int8_t> GetFsVerityDigest()
    {
        return fsVerityDigest;
    }

    std::vector<int8_t> GetDescriptorDigest()
    {
    ...
    void SetCsOffset(uint64_t& csOffset)
    {
...
private:
    ...
    std::vector<int8_t> descriptorDigest;
